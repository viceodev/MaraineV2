...
@if(count($submits) > 0)
    <section class="container capitalize p-1 fade">
        ...
        @foreach ($submits as $submit)
            <a href="{{route('admin.submissions.user', $submit['id'])}}">
                <ul class="bg-white shadow-ts p-1 user-container rounded flex-around align-items-center" style="margin: 20px auto;">
                    ...
                    <li>
                        <ul class="capitalize flex-rep flex-around align-items-center">
                            <li class="info card center">Course: {{$submit['course']}}</li>
                            <li class="info card center">Assignment Id: {{$submit['assignment_id']}}</li>
