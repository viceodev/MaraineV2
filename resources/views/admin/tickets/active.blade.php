...
@section('customBody')
    <meta id="page" content="{{$status['status']}}">
    <section class="list">
        <style>
        ...
        @if(count($status[$status['status']]) > 0 )
        @foreach ($status[$status['status']] as $ticket)
            
        <a href="{{route("admin.tickets.single", $ticket['id'])}}">
            <ul class="flex-between assign">
                <li class="name">{{$ticket['related']}}</li>
                <li class="li-title">{{$ticket['subject']}}</li>
                <li class="date" id="ticket-date">{{$ticket['created_at']}}</li>
